#!/usr/bin/env bash
#
# Run the cross-SDK chaos harness against sdk-go (bead qfg-47c2.4).
#
...
# 4. Runs `go test -tags chaos -run TestChaos ./...`.
...
#   GO_TEST_TIMEOUT          go test -timeout value (default 60m — scenarios 5/6/11 are slow)
#
# Examples:
#   ./scripts/run-chaos.sh
#   CHAOS_ONLY=02,05,07,09 ./scripts/run-chaos.sh   # red proof only
#   CHAOS_SKIP=11           ./scripts/run-chaos.sh   # skip the 30-min null-hypothesis

set -euo pipefail

HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SDK_GO_DIR="$(cd "$HERE/.." && pwd)"
REPO_ROOT="$(cd "$SDK_GO_DIR/.." && pwd)"
...
API_BIN="$SDK_GO_DIR/.chaos-api-delivery"
...
CHAOS_API_DELIVERY_URL="http://127.0.0.1:$API_PORT" \
  go test -tags chaos -run TestChaos -timeout "$GO_TEST_TIMEOUT" -v ./...
